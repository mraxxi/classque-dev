  ...
  return (
    <div className="flex flex-col h-full bg-gray-50 pb-20">
      ...
      <div className="flex border-b border-gray-200 bg-white overflow-x-auto">
        <button 
          className={`flex-1 min-w-[70px] py-3 text-sm font-medium ${activeTab === 'learners' ? 'text-indigo-600 border-b-2 border-indigo-600' : 'text-gray-500'}`}
          onClick={() => setActiveTab('learners')}
        >
          {t('groups.tabs.learners')}
        </button>
        <button 
          className={`flex-1 min-w-[70px] py-3 text-sm font-medium ${activeTab === 'rules' ? 'text-indigo-600 border-b-2 border-indigo-600' : 'text-gray-500'}`}
          onClick={() => setActiveTab('rules')}
        >
          {t('groups.tabs.schedule')}
        </button>
        <button 
          className={`flex-1 min-w-[70px] py-3 text-sm font-medium ${activeTab === 'sessions' ? 'text-indigo-600 border-b-2 border-indigo-600' : 'text-gray-500'}`}
        ...
        {isModuleEnabled('notes') && (
          <button 
            className={`flex-1 min-w-[70px] py-3 text-sm font-medium ${activeTab === 'notes' ? 'text-indigo-600 border-b-2 border-indigo-600' : 'text-gray-500'}`}
          ...
          <div className="space-y-3">
            {learners?.map((learner: any) => (
              <Link
                ...
                className="block bg-white p-3 rounded-lg shadow-sm border border-gray-200 hover:border-blue-400 transition-colors"
              >
      ...
      {isModuleEnabled('notes') && (
        <FloatingAddNoteButton
          groupId={id}
          onNoteAdded={() => queryClient.invalidateQueries({ queryKey: ['notes', 'group', id] })}
        />
      )}
    </div>
  );
